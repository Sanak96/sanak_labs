...
'''
def max_min():
    df['year'][df['VHI'] == df['VHI'].max()]
'''

'''
print os.getcwd()
url = "http://www.star.nesdis.noaa.gov/smcd/emb/vci/gvix/G04/ts_L1/ByProvince/Mean/L1_Mean_UKR.R16.txt"
vhi_url = urllib2.urlopen(url)
...
def download(id):
    ...
    url = "http://www.star.nesdis.noaa.gov/smcd/emb/vci/gvix/G04/ts_L1/ByProvince/Mean/L1_Mean_UKR.R{0}.txt".format(id)
    vhi_url = urllib2.urlopen(url)
    out = open('vhi_id_{0}_{1}.csv'.format(id, t), 'wb')
    out.write(vhi_url.read())
    out.close()
    print "VHI {0} is downloaded...".format(id)
'''